...
const BIG_NUMBER_SAVE_VALUE = 11;
...
function registerBigNumber(variableName)
{
    registerVariable(variableName, BIG_NUMBER_SAVE_VALUE);
}
    ...
    if(variableRegistry.length > index && saves[chosenSave].length > index)
    {
        ...
        switch(variableFormat)
        {
            ...
            case BOOL_ARRAY_SAVE_VALUE:
                ...
                break;
            case BIG_NUMBER_SAVE_VALUE:
                if(loadedValue.includes("."))
                {
                    loadedValue = loadedValue.substring(0, loadedValue.indexOf('.'));
                }
                console.log(variableName + " = " + loadedValue);
                eval(variableName + " = new BigNumber(" + loadedValue + ")");
                break;
...
function getFormattedVariableForSave(variableName)
{
    ...
    switch(variableFormat)
    {
        ...
        case BOOL_ARRAY_SAVE_VALUE:
            ...
            return result;
        case BIG_NUMBER_SAVE_VALUE:
            return eval(variableName).toString();
        case RAW_INTEGER_SAVE_VALUE:
            return String(eval(variableName));
        case JSON_SAVE_VALUE:
...
function loadAllVariables(chosenIndex)
{
    for(var i = 0; i < variableRegistry.length; i++)
    {
        ...
        {
            var variableName = variableRegistry[i][0]
            console.error("Failed to load variable '" + variableName + "'. Saved value = " + saves[chosenIndex][i]);
...
function runPostLoadingFixes()
{
    ...
    if(money.isNaN() || money.lessThan(0))
    {
    ...
    {
        earthTradeOffer1[TRADE_INDEX_PAYMENT_AMOUNT] = new BigNumber(earthTradeOffer1[TRADE_INDEX_PAYMENT_AMOUNT]);
        earthTradeOffer1[TRADE_INDEX_REWARD_AMOUNT] = new BigNumber(earthTradeOffer1[TRADE_INDEX_REWARD_AMOUNT]);
    }
    ...
    {
        earthTradeOffer2[TRADE_INDEX_PAYMENT_AMOUNT] = new BigNumber(earthTradeOffer2[TRADE_INDEX_PAYMENT_AMOUNT]);
        earthTradeOffer2[TRADE_INDEX_REWARD_AMOUNT] = new BigNumber(earthTradeOffer2[TRADE_INDEX_REWARD_AMOUNT]);
    }
    ...
    {
        moonTradeOffer1[TRADE_INDEX_PAYMENT_AMOUNT] = new BigNumber(moonTradeOffer1[TRADE_INDEX_PAYMENT_AMOUNT]);
        moonTradeOffer1[TRADE_INDEX_REWARD_AMOUNT] = new BigNumber(moonTradeOffer1[TRADE_INDEX_REWARD_AMOUNT]);
    }
    ...
    {
        moonTradeOffer2[TRADE_INDEX_PAYMENT_AMOUNT] = new BigNumber(moonTradeOffer2[TRADE_INDEX_PAYMENT_AMOUNT]);
        moonTradeOffer2[TRADE_INDEX_REWARD_AMOUNT] = new BigNumber(moonTradeOffer2[TRADE_INDEX_REWARD_AMOUNT]);
    }
    ...
    if(oldversion < 144)
    {
        activeExcavations.forEach(excavation =>
        {
    ...
    if(limitedTimeEventManager.activeEventType == EVENT_NONE || limitedTimeEventManager.activeEventType != previouslyActiveHolidayIndex)
    {
        worldResources[HOLIDAY_RESOURCE_INDEX].numOwned = 0;
        previouslyActiveHolidayIndex = limitedTimeEventManager.activeEventType;
    }
...
{
    registerBigNumber("money");
    registerInteger("depth");
    registerBigNumber("progressTowardsNextDepth");
    registerInteger("UID");
    ...
    registerInteger("worldResources[68].numOwned");
    registerInteger("previouslyActiveHolidayIndex");
    registerInteger("lastFlashQuestionPromptClickTime");
    registerString("offerManager.saveState");
    registerInteger("reducedOrangeFishLevels");
    registerBoolean("dronesReturnOnFull");
    registerBoolean("displayHiddenExcavations");
    registerBoolean("drillEquips[124].isCrafted");
    registerBoolean("drillEquips[125].isCrafted");
    registerBoolean("drillEquips[126].isCrafted");
    registerBoolean("drillEquips[127].isCrafted");
    registerBoolean("drillEquips[128].isCrafted");
    registerBoolean("drillEquips[129].isCrafted");
    registerBoolean("drillEquips[130].isCrafted");
    registerBoolean("drillEquips[131].isCrafted");
    registerBoolean("drillEquips[132].isCrafted");
    registerBoolean("drillEquips[133].isCrafted");
    registerBoolean("drillEquips[134].isCrafted");
    registerBoolean("drillEquips[135].isCrafted");
    registerString("purchaseHistory.saveState");
    registerString("mobileAdReward.saveState")
    registerString("rewardedChestStorage.saveState")
...
function loadGame(x)
{
    ...
    if(x < RSc)
    {
        ...
        if(!isMobile())
        {
            document.getElementById('WRAPPERD').style.background = 'url("Shared/Assets/UI/gui2.png")';
        ...
        if(isMobile()) 
        {
            activeLayers["FooterUILayer"] = new FooterUILayer({x: 0, y: mainh - mobileFooterH, width: mobileFooterW, height: mobileFooterH});
        }

        if(isConfirmationPromptOpen())
        {
            // If an alert popup was created before the main UI layers were initialized,
            // we need to re-add it to activeLayers so it isn't buried under the others
            for(var layerName in activeLayers)
            {
                if(layerName.includes("confirmationLayer") && activeLayers[layerName].boundingBox.width > 0)
                {
                    activeLayers[layerName + "_new"] = activeLayers[layerName];
                    activeLayers[layerName + "_new"].layerName = layerName + "_new";
                    delete activeLayers[layerName];
                }
            }
        }

        limitedTimeEventManager.onGameLoad();
...
function downloadBackup(onSuccessCallback = null, onErrorCallback = null)
{
    ...
    platform.steamApi.readTextFromFile(saveBackupFileName, onSuccess, onError);
}
...
function updateCloudBackup()
{
    ...
    platform.steamApi.saveTextToFile(saveBackupFileName,
...
function displayCloudBackups()
{
    if(document.getElementById("cloudBackupsUI") == null)
    {
        ...
        cloudBackupsUIExitButton.style.background = "url('Shared/Assets/UI/closei.png') no-repeat center";
...
function addBackupEntry(key, save, backupTime)
{
    ...
    if(progressData.money.includes("."))
    {
        progressData.money = progressData.money.substring(0, progressData.money.indexOf('.'));
    }

    var progressString = _("Depth") + ": " + progressData.depth + "km<br>" + _("Money") + ": " + beautifynum(new BigNumber(progressData.money));
...
function isSaveFileLoaded()
{
    return typeof (chosen) != "undefined" && chosen >= 0;
}
