...
function learnReachedBlueprints()
{
...
{
    var blueprint = getBlueprintById(category, blueprintId);
    if(blueprint.price && money.lessThan(blueprint.price))
    {
...
{
    return doBigNumberDecimalMultiplication(blueprint.price, STAT.blueprintPriceMultiplier());
}
...
function filterBlueprints(blueprintArray, checkFunction)
{
    ...
    for(var i in blueprintArray)
    {
        ...
        {
            var blueprintCategoryAndId = blueprint.split(".");
...
function generateHtmlForIngredients(ingredients, showSufficiencyIconsAndText = true)
{
    ...
    for(var i in rawIngredients)
    {
        ...
        if(showSufficiencyIconsAndText)
        {
            ...
            var sufficiencyImageDesignator = rawIngredients[i].hasQuantity ? "Shared/Assets/UI/checkmark2b.png" : "Shared/Assets/UI/xmark.png";
...
{
    learnRangeOfBlueprints(craftingCategories.miners, 0, craftingBlueprints[craftingCategories.miners].length - 1);
    learnRangeOfBlueprints(craftingCategories.minerLevels, 0, craftingBlueprints[craftingCategories.minerLevels].length - 1);
...
function getIngredientListWithDiscounts(ingredients, isMiner = false)
{
    ...
    for(var i in ingredients)
    {
        discountedIngredients[i] = {
            ...
            quantity: getDiscountedIngredientQuantity(ingredients[i], isMiner)
        }
    }
    return discountedIngredients;
}

function getRealBlueprintCost(ingredients)
{
    let total = new BigNumber(0);

    ingredients.forEach(ingredient =>
    {
        let quantity = new BigNumber(ingredient.quantity);
        let sellValue;
        if (ingredient.item instanceof MoneyCraftingItem || !ingredient.hasOwnProperty("sellValue"))
        {
            sellValue = new BigNumber(1);
        }
        else
        {
            sellValue = new BigNumber(ingredient.sellValue);
        }
        if (sellValue.equals(0))
        {
            total = total.add(quantity.multiply(60 * 30).multiply(valueOfMineralsPerSecond()));
        }
        else
        {
            total = total.add(quantity.multiply(sellValue));
        }
    })
...
function getDiscountedIngredientQuantity(ingredient, isMiner = false)
{
    if(ingredient.item instanceof MoneyCraftingItem)
    {
        if(isMiner)
        {
            return doBigNumberDecimalMultiplication(ingredient.quantity, STAT.minerUpgradeAndHireCostMultiplier());
        }
        else
        {
            return doBigNumberDecimalMultiplication(ingredient.quantity, STAT.blueprintPriceMultiplier());
        }
    }
    else
    {
