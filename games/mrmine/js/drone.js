...
class ActiveDrone extends BaseDrone
{
    ...
    constructor(baseDrone, caveSystem, nodePath, level = -1)
    {
        ...
        this.speedMultiplier = this.speedMultiplierLevels[this.level] * STAT.droneSpeedMultiplier();
        ...
        this.totalFuel = Math.floor(this.totalFuel * STAT.startingFuelMultiplier())
    ...
    update(deltaTime)
    {
        ...
        if(!this.isFinished && this.isAlive)
        {
            ...
            if(this.baseDroneId == 0 && this.isInventoryFull() && this.rewardCapacity != 0 && dronesReturnOnFull && this.isMovingForward)
            {
                this.turnAround();
            }
        }
    ...
    turnAround()
    {
        this.nodePath = this.nodePath.slice(0, this.lastReachedNodeIndex + 2);
        this.lastReachedNodeIndex = this.lastReachedNodeIndex + 1;
        this.progressToNextNode = 1 - this.progressToNextNode;
    ...
    handlePathCompletion()
    {
        ...
        this.inventory = [];
        let fuelToReturn = Math.floor(Math.min(this.currentFuel, drones[this.baseDroneId].totalFuel))
        this.caveSystem.addFuel(fuelToReturn);
...
var magneticDrone = new BaseDrone();
magneticDrone.id = 1;
...
var basicFlyingDrone = new BaseDrone();
...
var healingDrone = new BaseDrone();
healingDrone.id = 3;
