class BattleWindow extends PopupWindow
{
    ...
    constructor(boundingBox, monster)
    {
        ...
        for(var i = 0; i < 12; i++)
        {
            ...
            this.addHitbox(new Hitbox(
                {
                    x: this.boundingBox.width * (0.0921 + ((column) * 0.3025)),     // Copied from renderButton call below
                    y: this.boundingBox.height * (.692 + (.062 * (entry - 1))),
                    width: this.boundingBox.width * .2424,
                    height: this.boundingBox.height * .051
                },
                {
    ...
    render()
    {
        ...
        for(var bi = 0; bi < 12; bi++)
        {
            ...
            if(battleInventory[bi])
            {
                if(battleInventory[bi][0] > -1)
                {
                    ...
                    var pcentAtk = 0;
                    let padding = column == 3 ? .0015 : 0;
                    var weaponChargeX = this.boundingBox.width * (0.0921 + ((column - 1) * 0.3025) + padding);
                    var weaponChargeY = this.boundingBox.height * (.692 + (.062 * (entry - 1)));
                    var weaponChargeWidth = this.boundingBox.width * .2424;
                    ...
                    this.context.drawImage(getInventoryWeaponIcon(bi), 0, 0, 50, 50, this.boundingBox.width * (0.04 + ((column - 1) * 0.304)), weaponChargeY, this.boundingBox.width * .045, this.boundingBox.height * .0525);
                    if(isWeaponCharged)
                    {
        ...
        {
            this.context.fillText("Wild " + this.monster.name, this.boundingBox.width * .055, this.boundingBox.height * .1);
        }
        else
        {
            this.context.fillText(this.monster.name, this.boundingBox.width * .055, this.boundingBox.height * .1);
        }
