        ...
        for(var i = 0; i < superMinerManager.numSuperMiners(); i++)
        {
            ...
            this.superMinerHitboxes.push(this.superMinerPane.addHitbox(new Hitbox(
                ...
                },
                {
                    onmousedown: () =>
                    {
                        if(keysPressed["Shift"] && superMiner.canPressButton())
                        {
                            superMiner.onButtonPress();
                        }
                        else if(!this.showScrapButtons)
                        {
                ...
                onmousedown: () =>
                {
                    if(superMinerManager.canUpgradeSlot())
                    {
                        showConfirmationPrompt(
                            ...
                            _("Cancel"),
                            null,
                            _("NOTE: This only adds slots, it does not grant Super Miners.")
                        )
                    }
                    else if(!activeLayers.SuperMinerSelectorWindow)
                    {
                        showConfirmationPrompt(
                            _("You don't have enough Super Miner Souls. Open more Ethereal Chests or scrap Super Miners", superMinerManager.nextSlotCost()),
                            _("Open Ethereal Chests"),
                            () =>
                            {
