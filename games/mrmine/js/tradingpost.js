...
const TIME_VALUE_PER_TICKET = 7200;
...
function determineTradeAmounts(paymentType, paymentSubtype, rewardType, rewardSubtype, worldIndex)
{
    ...
    if(rewardType == TRADE_TYPE_CHEST || rewardType == TRADE_TYPE_BLUEPRINT ||
        ...
        rewardType == TRADE_TYPE_TICKETS)
    {
        rewardTimeValue = determineTimeValueOfTradeType(rewardType, rewardSubtype);
        paymentTimeValue = rewardTimeValue.divide(valueGain);
        if(rewardType == TRADE_TYPE_BLUEPRINT)
        {
            var blueprintPrice = craftingBlueprints[1][rewardSubtype].price;
            paymentTimeValue = parseInt(getMoneyBasedTimeValue(paymentTimeValue, valueGain, blueprintPrice, 1));
        }
    }
    else
    {
        paymentTimeValue = determineTimeValueOfTradeType(paymentType, paymentSubtype);
        rewardTimeValue = doBigNumberDecimalMultiplication(paymentTimeValue, valueGain);
    }
...
{
    var value1 = valueOfMineralsPerSecond().multiply(paymentTimeValue).divide(valueGain);
    var value2 = rewardSubtypeValue.multiply(valueFraction);
    var value3 = bigNumberMax(
        money.add(getValueOfMinerals()),
        value2
    ).divide(2);
    var minValue = bigNumberMin(value1, value2, value3);
    var maxValue = bigNumberMax(value1, value2, value3);
    var middleValue = value1.add(value2).add(value3).subtract(minValue).subtract(maxValue);
    var timeValue = tradeRoller.randBigNumber(middleValue, maxValue).divide(valueOfMineralsPerSecond());
    return timeValue;
}
...
function determineTimeValueOfTradeType(tradeType, tradeSubtypeIndex = null)
{
    ...
    switch(tradeType)
    {
        ...
        case TRADE_TYPE_RELIC:
            ...
            {
                resultTimeValue = 36000 * Math.pow(excavationRewards[tradeSubtypeIndex].rarity, 2.5);
            }
            break;
        case TRADE_TYPE_TICKETS:
            resultTimeValue = tradeRoller.rand(1, 2) * TIME_VALUE_PER_TICKET;
            break;
        default:
    ...
    {
        return new BigNumber(parseInt(playtime / 2));
    }
    return new BigNumber(resultTimeValue);
}
...
function calculateAmountOfTradeType(tradeType, tradeSubtype, timeValue, isPayment, otherType = null, otherSubtype = null, otherAmount = null)
{
    ...
    switch(tradeType)
    {
        ...
        case TRADE_TYPE_RELIC:
            return new BigNumber(1);
        case TRADE_TYPE_TICKETS:
            return new BigNumber(Math.ceil(timeValue / TIME_VALUE_PER_TICKET));
        case TRADE_TYPE_ORE:
            if(!isPayment && otherType == TRADE_TYPE_MONEY)
            {
                return new BigNumber(timeValue).multiply(valueOfMineralsPerSecond()).divide(worldResources[tradeSubtype].sellValue);
            }
            else if(!isPayment && otherType == TRADE_TYPE_ORE)
            {
                var otherValue = new BigNumber(timeValue).multiply(valueOfMineralsPerSecond());
                var quantity = otherValue.divide(worldResources[tradeSubtype].sellValue);
                if(quantity.lessThan(0)) quantity.setValue(1);
                return quantity;
            }
            else if(isPayment && otherType == TRADE_TYPE_RELIC)
            {
                var otherValue = new BigNumber(timeValue).multiply(valueOfMineralsPerSecond());
                var quantity = otherValue.divide(worldResources[tradeSubtype].sellValue);
                if(quantity.lessThanOrEqualTo(0)) quantity.setValue(1);
                return quantity;
            }
            else
            {
                ...
                return doBigNumberDecimalMultiplication(new BigNumber(time), mineral);
            }
        case TRADE_TYPE_MONEY:
            return new BigNumber(valueOfMineralsPerSecond()).multiply(timeValue);
        default:
            throw "Invalid trade type";
    }
}
...
function canMakeTrade(tradeOffer, showMessageOnFail = false)
{
    ...
    {
        canMakeTrade = isTradeAvailable(tradeOffer) &&
            money.greaterThanOrEqualTo(tradeOffer[TRADE_INDEX_PAYMENT_AMOUNT]);
        message = _("You can't afford that trade.");
    }
...
function makeTrade(worldIndex, tradeOffer)
{
    ...
    switch(tradeOffer[TRADE_INDEX_REWARD_TYPE])
    {
        case TRADE_TYPE_ORE:
            ...
            let value = new BigNumber(tradeOffer[TRADE_INDEX_REWARD_AMOUNT]).multiply(worldResources[tradeOffer[TRADE_INDEX_REWARD_SUBTYPE]].sellValue);
        ...
        case TRADE_TYPE_BLUEPRINT:
            ...
            break;
        case TRADE_TYPE_TICKETS:
            addTickets(parseInt(tradeOffer[TRADE_INDEX_REWARD_AMOUNT]), "trade");
            break;
        default:
            return false;
    }
    ...
    switch(tradeOffer[TRADE_INDEX_PAYMENT_TYPE])
    {
        ...
        case TRADE_TYPE_ORE:
            ...
            else
            {
                ...
                paymentValueString = "$" + beautifynum(worldResources[tradeOffer[TRADE_INDEX_PAYMENT_SUBTYPE]].sellValue.multiply(tradeOffer[TRADE_INDEX_PAYMENT_AMOUNT]));
            }
            break;
    }
    switch(tradeOffer[TRADE_INDEX_REWARD_TYPE])
    {
        ...
        case TRADE_TYPE_ORE:
            ...
            else
            {
                ...
                rewardValueString = "$" + beautifynum(worldResources[tradeOffer[TRADE_INDEX_REWARD_SUBTYPE]].sellValue.multiply(tradeOffer[TRADE_INDEX_REWARD_AMOUNT]));
            }
            break;
        case TRADE_TYPE_CHEST:
        ...
        case TRADE_TYPE_BUFF:
            ...
            break;
        case TRADE_TYPE_TICKETS:
            if(tradeOffer[TRADE_INDEX_REWARD_AMOUNT] == 1)
            {
                rewardString = _("1 ticket");
            }
            else
            {
                rewardString = _("{0} tickets", tradeOffer[TRADE_INDEX_REWARD_AMOUNT]);
            }
            rewardValueString = _("Unknown");
            break;
    }
...
function setFirstTrade()
{
    ...
    tradeArrays[0][TRADE_INDEX_PAYMENT_AMOUNT] = new BigNumber(1000);
    tradeArrays[0][TRADE_INDEX_REWARD_TYPE] = TRADE_TYPE_ORE;
    tradeArrays[0][TRADE_INDEX_REWARD_SUBTYPE] = GOLD_INDEX;
    tradeArrays[0][TRADE_INDEX_REWARD_AMOUNT] = new BigNumber(20);
    ...
    tradeArrays[1][TRADE_INDEX_PAYMENT_AMOUNT] = new BigNumber(2000);
    tradeArrays[1][TRADE_INDEX_REWARD_TYPE] = TRADE_TYPE_CHEST;
    tradeArrays[1][TRADE_INDEX_REWARD_SUBTYPE] = 0;
    tradeArrays[1][TRADE_INDEX_REWARD_AMOUNT] = new BigNumber(1);
    ...
    [
        {
            ...
            "introDialogue": [
                ...
                _("They might call me Lil Nuggz but there isn't anything little about my wallet."),
...
function testFrequencyOfTradeReward(rewardType, testsToRun = 20, tradesPerTest = 100)
{
    let trades = {
        [EARTH_INDEX]: new Array(testsToRun).fill(0),
        [MOON_INDEX]: new Array(testsToRun).fill(0),
    };

    for(var x = 0; x < testsToRun; x++)
    {
        for(var i = 0; i < tradesPerTest; i++)
        {
            generateTrade(EARTH_INDEX);
            generateTrade(MOON_INDEX);
            let earthTrades = getTradesForWorld(EARTH_INDEX);
            let moonTrades = getTradesForWorld(MOON_INDEX);

            if(earthTrades[0][TRADE_INDEX_REWARD_TYPE] == rewardType) trades[EARTH_INDEX][x]++;
            if(earthTrades[1][TRADE_INDEX_REWARD_TYPE] == rewardType) trades[EARTH_INDEX][x]++;
            if(moonTrades[0][TRADE_INDEX_REWARD_TYPE] == rewardType) trades[MOON_INDEX][x]++;
            if(moonTrades[1][TRADE_INDEX_REWARD_TYPE] == rewardType) trades[MOON_INDEX][x]++;
        }
    }

    console.log("Earth Trades of Type: " + calculateAverageOfArray(trades[EARTH_INDEX]));
    console.log("Moon Trades of Type: " + calculateAverageOfArray(trades[MOON_INDEX]));
}