...
class WanderingSuperMinerRenderer
{
    ...
    {
        let x = 0;
        let platformDepth = isMobile() ? this.parent.currentDepth + 1 : this.parent.currentDepth;
        let y = worldConfig.topBound + worldConfig.levelHeight *
...
class DrillMinerRenderer
{
    ...
    render(context)
    {
        if(currentlyViewedDepth >= depth - 2)
        {
            ...
            {
                var yOffset = 0.3; // level depths
                x = (mainw - minerWidth) * 0.5;
                y = worldConfig.topBound + worldConfig.levelHeight *
                    (yOffset + worldConfig.numberOfDepthsVisible + depth - (currentlyViewedDepth + partialDepthOffset))
                    - mainh * worldConfig.bottomBoundFraction;
            }
