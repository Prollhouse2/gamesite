...
function generateIdleQuote()
{
    ...
    if(!isDepthWithoutWorkers(quoteDepth) && !BattleManager.isActiveBossLevel(quoteDepth))
    {
...
var workerQuotes = [
    ...
    {
        "id": 138,
        "quote": "Only coal for christmas this year :(",
        "canShow": isChristmasCoalMiner,
        "weight": SHOW_FREQUENT,
        "nextIdsToShow": []
    },
    {
        "id": 139,
        "quote": "I ate way too much turkey",
        "canShow": isThanksgiving,
        "weight": SHOW_FREQUENT,
        "nextIdsToShow": []
    },
    {
        "id": 140,
        "quote": "I wish I could go see the New Year fireworks.",
        "canShow": isNewYears,
        "weight": SHOW_FREQUENT,
        "nextIdsToShow": []
    },
    {
        "id": 141,
        "quote": "You won't believe the black friday deal I got on my new pickaxe!",
        "canShow": isBlackFriday,
        "weight": SHOW_FREQUENT,
        "nextIdsToShow": []
    },
    {
        "id": 142,
        "quote": "Wow he found a present!",
        "canShow": isNearChestOnChristmas,
        "weight": SHOW_FREQUENT,
        "nextIdsToShow": []
    }
];
...
{
    return BattleManager.isActiveBossLevel(queriedDepth + 1);
}
...
{
    return money.greaterThanOrEqualTo(1000000000000);
}
...
function isChristmasCoalMiner(queriedDepth, minerNum)
{
    return limitedTimeEventManager.isXmas() && queriedDepth < 10;
}

function isThanksgiving(queriedDepth, minerNum)
{
    return limitedTimeEventManager.isThanksgiving();
}

function isNewYears(queriedDepth, minerNum)
{
    return limitedTimeEventManager.isNewYears();
}

function isBlackFriday(queriedDepth, minerNum)
{
    return limitedTimeEventManager.isBlackFriday();
}

function isNearChestOnChristmas(queriedDepth, minerNum)
{
    return limitedTimeEventManager.isXmas() && chestService.chestExistsAtDepth(queriedDepth);
}