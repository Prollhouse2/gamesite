class CaveSystem
{
    ...
    constructor(kmDepth, caveTreeDepth, maxNodesPerDepth)
    {
        ...
        let baseTime = (CAVE_SYSTEM_DURATION_PER_NODE_SECONDS * this.numNodes());
        let depthTimeReduction = (1 + ((depthMultiplier() - 1) * .33));
        let roundedToMinTime = Math.round(((baseTime / depthTimeReduction) / 60) * 60);
        this.totalDuration = Math.max(CAVE_MINIMUM_LIFETIME_SECONDS, roundedToMinTime);
    ...
    startDroneOnPath(baseDrone, nodePath)
    {
        if(this.canCraftDrone(baseDrone))
        {
            ...
            if(!this.isExplored)
            {
                ...
                trackEvent_StartCave(this.kmDepth);
            }
            return newDrone;
        }
        return null;
