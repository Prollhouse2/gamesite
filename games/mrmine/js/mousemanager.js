...
{
    mouseMoved = true;
    if(!platform.treatClicksAsTouches || isMouseDown)
    {
        prevMouseX = mouseX; // Global vars
        prevMouseY = mouseY;
        if(event.touches)
...
{
    if (event.target.classList.contains("languageSelection"))
    {
        return;
    }
    isMouseDown = true;
...
function disableMouseEvents(targetDiv, duration = 0)
{
    if (targetDiv.style && targetDiv.style.pointerEvents != "none")
    {
        var originalValue = targetDiv.style.pointerEvents;
        targetDiv.style.pointerEvents = "none";
        if (duration > 0)
        {
            setTimeout(() => {
                targetDiv.style.pointerEvents = originalValue;
            }, duration);
        }
    }
}

function disableOnClickHtmlEvent(targetDiv, duration = 0)
{
    if (targetDiv.onclick)
    {
        var originalValue = targetDiv.onclick;
        targetDiv.onclick = null;
        if (duration > 0)
        {
            setTimeout(() => {
                targetDiv.onclick = originalValue;
            }, duration);
        }
    }
}