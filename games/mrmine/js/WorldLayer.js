...
class WorldLayer extends UiLayer
{
    ...
    renderTopCity()
    {
        if(currentlyViewedDepth < 4)
        {
            ...
            if(currentlyViewedDepth < 2)
            {
                ...
                {
                    MAIN.drawImage(hat5, 64, 0, 32, 48, Math.ceil(mainw * .630), Math.ceil(mainh * (.3365 - (.179 * currentlyViewedDepth))), Math.floor(mainw * .028), Math.floor(mainh * .064));
                }
                else if(limitedTimeEventManager.isHalloween())
                {
                    MAIN.drawImage(witchhat5, 0, 0, 32, 48, Math.ceil(mainw * .625), Math.ceil(mainh * (.336 - (.179 * currentlyViewedDepth))), Math.floor(mainw * .032), Math.floor(mainh * .074));
                }
            }
            ...
            if(currentlyViewedDepth == 0)
            {
                if(limitedTimeEventManager.isXmas())
                {
                    for(var i = 0; i < 17; i++)
                    {
                        MAIN.drawImage(xmasLights, ((i + Math.floor(framesRendered2 / 4)) % 4) * xmasLights.width / 4, 0, xmasLights.width / 4, xmasLights.height / 3, mainw * .082 + (i * mainw * .052), mainh * .105, mainw * .052, mainh * .055);
                    }
                }

                MAIN.drawImage(discordb, 0, 0, 50, 50, Math.ceil(mainw * .085), Math.ceil(mainh * .12), Math.floor(mainw * .025), Math.ceil(mainh * .045));

                if(!isEmailCollected())
                {
                    MAIN.drawImage(emailb, 0, 0, emailb.width, emailb.height, Math.ceil(mainw * .085), Math.ceil(mainh * .1675), Math.floor(mainw * .025), Math.ceil(mainh * .045));
                }
                ...
                if(limitedTimeEventManager.isEventActive)
                {
                    ...
                    {
                        MAIN.drawImage(eventb, Math.ceil(mainw * .15), Math.ceil(mainh * .115), Math.floor(mainw * .13), Math.ceil(mainh * .08));
                        MAIN.save();
                        MAIN.font = "12px Verdana";
                        MAIN.fillStyle = "#FFF";
                        fillTextWrapWithHeightLimit(MAIN,
                ...
                if(depth > 20 && document.getElementById("surveyDiv") == null)
                {
                    if(!hasTakenSurveyCheck() && !(localStorage["isSurveyPending"] == "true" || localStorage["isSurveyPending"] == true))
                    {
                        MAIN.drawImage(surveyb, 0, 0, surveyb.width, surveyb.height, Math.ceil(mainw * .085), Math.ceil(mainh * .2150), Math.floor(mainw * .025), Math.ceil(mainh * .045));
                    }
    ...
    renderDrill()
    {
        if(currentlyViewedDepth >= depth - 1 && ((depth > 1000 && depth < 1032) || (depth > 1783 && depth < 1813)) && !BattleManager.isStalledDueToBoss() && !isWaitingForLiftoff())
        {
        ...
        else if(currentlyViewedDepth == depth)
        {
            ...
            MAIN.fillRect(Math.ceil(mainw * .815), Math.ceil(mainh * .72), Math.ceil(mainw * .111) * Math.min(1, (divideBigNumberToDecimalNumber(progressTowardsNextDepth, depthDifficultyTable[depth]))), Math.ceil(mainh * .02));
    ...
    renderSingleLevelOfDepth(depthToRender)
    {
        ...
        if(!isDepthWithoutWorkers(depthToRender))
        {
            ...
            if(!BattleManager.isActiveBossLevel(depthToRender))
            {
    ...
    renderBackgroundForDepth(depthToRender, xCoordinateOfLevel, yCoordinateOfLevelTop)
    {
        if(depthToRender < 1000)
        {
            if(!isDepthWithoutWorkers(depthToRender) && !this.isTradingPost(depthToRender) && !this.isCore(depthToRender))
            {
                for(var i = 0; i < 52; i++)
                {
                    if(backgroundGemPattern[depthToRender % 15][i] > 0)
                    {
                        MAIN.drawImage(wallgems, 20 * ((backgroundGemPattern[depthToRender % 15][i] - 1) + ((levels[depthToRender][0][0] - 1) * 5)), 0, 20, 120, xCoordinateOfLevel + Math.floor(mainw * (i * .016)), yCoordinateOfLevelTop, Math.floor(mainw * .016), Math.ceil(mainh * .178));
                    }
                }
            }
        }
    ...
    renderBackgroundDecalsForDepth(depthToRender, xCoordinateOfLevel, yCoordinateOfLevelTop)
    {
        ...
        else if(depthToRender == 100 && (chestCollectorChanceStructure.level == 0 || chestCollectorStorageStructure.level == 0))
        {
        ...
        else if(depthToRender == 2039)
        {
            MAIN.drawImage(Titan_Robot, 40 * (Math.floor(numFramesRendered / 6) % 4), 0, 40, 160, xCoordinateOfLevel + (mainw * .043), yCoordinateOfLevelTop + (mainh * .053), Math.floor(mainw * .03), Math.ceil(mainh * .14));
        }
    }
    ...
    renderLevelImageForDepth(depthToRender, xCoordinateOfLevel, yCoordinateOfLevelTop)
    {
        ...
        else if(depthToRender < 1000)
        {
            ...
            {
                MAIN.drawImage(BattleManager.getBossLevelAsset(depthToRender), 0, 0, 926, 120, xCoordinateOfLevel, yCoordinateOfLevelTop, Math.floor(mainw * .724), Math.ceil(mainh * .178));
            }
        }
        else if(depth >= 1000)
        {
            ...
            else if(!BattleManager.isActiveBossLevel(depthToRender))
            {
                ...
                else if(depthToRender <= titan.endDepth)
                {
