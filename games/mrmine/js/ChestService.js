class ChestService
{
    ...
    totalMoneyFromChests = new BigNumber(0);
    ...
    spawnChestAtRandomDepth(source)
    {
        ...
        var isValidSpawnSettings = !isDepthWithoutWorkers(spawnDepth) && !BattleManager.isActiveBossLevel(spawnDepth) && miner;
    ...
    spawnChest(spawnDepth, source = Chest.natural, type = ChestType.basic, miner = -1)
    {
        ...
        let chanceOfCollision = (this.chests.length + 1) / (Math.min(150, depth / 10)); //this is here to simulate how chest spawns acted in the old system.
    ...
    migrateDeprecatedChestStorage()
    {
        ...
        {
            this.storeChest(this.rollForSpecialChest());
        }
        this.chestsStored = 0;
    }
    ...
    rewardMultiplier()
    {
        return depthMultiplier() + superMinerManager.getChestRewardMultiplier();
    }
    ...
    testChestSpawnsForTime(minutes)
    {
        ...
        let previousChestCollector = {level: chestCollectorChanceStructure.level, amount: chestCollectorChanceStructure.leveledStats[0].perLvlValue[9]};
        ...
        chestCollectorChanceStructure.level = previousChestCollector.level;
        chestCollectorChanceStructure.leveledStats[0].perLvlValue[9] = previousChestCollector.amount;
