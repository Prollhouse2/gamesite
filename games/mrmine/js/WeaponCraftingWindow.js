class WeaponCraftingWindow extends TabbedPopupWindow
{
    ...
    {
        var iconSize = 50;
        var padding = this.bodyContainer.boundingBox.width * .015;
        var detailsWidth = this.bodyContainer.boundingBox.width * .22;
        ...
        for(var i = 0; i < battleInventory.length; ++i)
        {
            ...
            {
                var xCoordinate = Math.floor(i / 4) * (iconSize + padding + detailsWidth);
                var yCoordinate = (i % 4) * (iconSize + padding);

                var equipInfo = new Hitbox(
                    {
                        x: xCoordinate,
                        y: yCoordinate,
                        width: iconSize,
                ...
                equipInfo.render = function (parentWindow, i)
                {
                ...
                }.bind(equipInfo, this, i);
                equipList.addHitbox(equipInfo);

                var equipHitbox = new WeaponGridCell(
                    {
                        x: xCoordinate,
                        y: yCoordinate,
                        width: iconSize,
                ...
                var weaponComponent = new WeaponComponentUI(
                    {
                        x: 0,
                        y: 0,
                        width: iconSize,
                ...
                weaponComponent.isOnGrid = true;
                equipHitbox.addHitbox(weaponComponent);
            }
        }
...
class WeaponGridCell extends DragDropUIComponent
{
    column;
    ...
    onAcceptDrop()
    {
        ...
        let draggedWeapon = battleInventory[draggedHitbox.index];
        let currentHitbox = this.hitboxes[0];
        let currentWeapon = battleInventory[this.index];
        ...
        battleInventory[this.index] = draggedWeapon;
        battleInventory[draggedHitbox.index] = currentWeapon;

        //should rewrite this so the cells don't flicker on rerender
        activeLayers.weaponcrafting.equipList.clearHitboxes();
        activeLayers.weaponcrafting.initializeEquipList();
...
class WeaponComponentUI extends DragDropUIComponent
{
    index;
    isMouseOver = false;
    isOnGrid;
    ...
    onmouseenter(e)
    {
        ...
        var item = new WeaponCraftingItem(battleInventory[this.index][0]);
        showTooltip(
            _(item.getName()),
    ...
    renderComponent()
    {
        ...
        var item = new WeaponCraftingItem(battleInventory[this.index][0]);
        rootContext.drawImage(item.getIcon(), localCoordinates.x, localCoordinates.y, this.boundingBox.width, this.boundingBox.height);
    }
}